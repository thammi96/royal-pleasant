...
function Invoke-WebClientSsoLogin {
    ...
    # Nach jeder Navigation pruefen: sind wir angemeldet? (URL = WebClient/Main,
    # nicht mehr SignIn/SAML) -> Cookies auslesen und Fenster schliessen.
            ...
            if ($src -match '/WebClient' -and $src -notmatch 'SignIn|SingleSignOn|login\.microsoftonline|sts\.windows') {
                ...
                $task.ContinueWith({
                    ...
                    try {
                        ...
                        # nur schliessen, wenn eine Session-Cookie (Auth) dabei ist
                        if ($list.Count -gt 0) {
    ...
    if (-not $script:WcState.Cookies -or $script:WcState.Cookies.Count -eq 0) {
        throw 'Anmeldung abgebrochen oder keine Session-Cookies uebernommen.'
    }
...
function Get-WebClientSession {
    ...
    if (-not (Test-WebClientSession $session)) {
        throw 'WebClient-Anmeldung fehlgeschlagen: Session nach SSO nicht gueltig.'
    }

...
function Get-TokenViaAuthCodePkce {
    ...
    $script:AcState = @{ Code = $null; Error = $null; ExpectedState = $state; CookieJar = $null; CookieHeader = $null }
    ...
    switch ($variant) {
        '1' { Write-DebugLog 'Token variant 1: faithful KeePass client emulation (X-Pleasant headers + session cookies).' }
        '2' {
            Write-DebugLog 'Token variant 2: like 1 but WITHOUT session cookies (isolates the cookie).'
            $postArgs.ExtraHeaders = $hdrClient
        }
        '3' {
            Write-DebugLog 'Token variant 3: like 1 but WITHOUT X-Pleasant headers (isolates the headers).'
            $h = @{}; if ($script:AcState.CookieHeader) { $h['Cookie'] = $script:AcState.CookieHeader }
            $postArgs.ExtraHeaders = $h
        }
        '4' {
            Write-DebugLog 'Token variant 4: plain body only, no extra headers (old v13/v14 behaviour).'
            $postArgs.ExtraHeaders = $null
        }
        '5' {
            Write-DebugLog 'Token variant 5: like 1 but client_id percent-encoded.'
            $postArgs.Body = ('client_id=' + [uri]::EscapeDataString($cid)) + "&$pGrant&$pCode&$pRuri&$pVer"
        }
        '6' {
            Write-DebugLog 'Token variant 6: like 1 + device_id/device_name in the body.'
            $postArgs.Body = "$bodyFull&device_id=$deviceId&device_name=" + [uri]::EscapeDataString($deviceName)
        }
        '7' {
            Write-DebugLog 'Token variant 7: like 1 with charset=utf-8.'
            $postArgs.ContentType = 'application/x-www-form-urlencoded; charset=utf-8'
        }
        '8' {
            Write-DebugLog 'Token variant 8: like 1 without redirect_uri.'
            $postArgs.Body = "$pCid&$pGrant&$pCode&$pVer"
        }
